...
module.exports = {
  prompts: [
    ...
    },
    {
      ...
      cliName: 'enable-quality-checks',
      message: 'Should configure and enable checks (findbugs, pmd, checkstyle)',
      required: true,
      default: true,
      store: true
    }],
  ...
  write: function write(generator) {
    ...
    generator.fs.copyTpl(
      generator.templatePath('build.gradle'),
      'build.gradle',
      {
        enableQualityChecks: generator.props.enableQualityChecks
      }
    );
    ...
    generator.fs.copy(generator.templatePath('gradle/wrapper'), 'gradle/wrapper');
    ...
    console.log('generator.props.enableQualityChecks: ', generator.props.enableQualityChecks);

    if (generator.props.enableQualityChecks) {
      mkdirp('config');
      generator.fs.copy(generator.templatePath('config/checkstyle'), 'config/checkstyle');
      generator.fs.copy(generator.templatePath('config/pmd'), 'config/pmd');
      generator.fs.copyTpl(
        generator.templatePath('config/findbugs'),
        'config/findbugs',
        {
          escapedBasePackageName: generator.props.basePackageName.replace('.', '\\.')
        }
      );

      generator.fs.copy(generator.templatePath('quality.gradle'), 'quality.gradle');
    }
